...
elite athlete's recovery. Sprint speed declined in two clear steps,
one after each surgery, and is still declining in 2026. Stolen base
success rate does not follow the same pattern — it hit a career-best
90% the year after his second surgery, then dropped sharply this
season.**

Part of the [Emerson Performance](https://github.com/ejimenezperformance) analytics portfolio (EP-TSP framework). This is the lower-body mirror of `tj-recovery-trajectory` — same return-to-play question, applied to knee
injury instead of elbow, and to baserunning instead of pitching.

*[Versión en español disponible aquí](https://github.com/ejimenezperformance/acuna-acl-recovery/blob/main/README.es.md)*

---

## Finding 1 — Sprint speed: peaked before either injury, then declined in two steps

![Sprint speed trajectory](https://github.com/ejimenezperformance/acuna-acl-recovery/raw/main/outputs/acuna_trajectory_EN.png)
...
| 2019                     | 29.3                 |
| 2020                     | 29.1                 |
| 2021 (1st ACL tear, Jul) | 29.4                 |
| 2022                     | 28.5                 |
| 2023 (40-70 season)      | 28.0                 |
| 2024 (2nd ACL tear, May) | 27.7                 |
| 2025                     | 27.9                 |
| 2026                     | 27.2                 |

Acuña's sprint speed peaked in 2018 (29.6 ft/s, "plus-plus" by Statcast
standards) and stayed elite right through his first ACL tear — his 2021
mark (29.4 ft/s), the year of that injury, is his second-highest on
record. The decline shows up in two clear steps after each surgery:
down to a 28.0–28.5 plateau following the first tear, then down again
to a 27.2–27.9 range following the second. 2026 (27.2 ft/s) is his
lowest mark yet and does not show signs of stabilizing.

## Finding 2 — Stolen base success rate tells a very different story

![Stolen base success rate](https://github.com/ejimenezperformance/acuna-acl-recovery/raw/main/outputs/acuna_sb_success_EN.png)
...
post-injury decline. His best-ever success rate (90.0%) came the season *after* his second ACL surgery — his lowest-speed season on record — not
...
*Note: 2026 SB/CS counts above reflect the snapshot this analysis was
pulled at (through roughly mid-season) — check Baseball-Reference for
the current in-season total before citing this number as final.*

## Finding 3 — 2026: a career-high lead, the same year success dropped

![Lead distance](https://github.com/ejimenezperformance/acuna-acl-recovery/raw/main/outputs/acuna_lead_distance_EN.png)
...
found repeatedly on the hitting and pitching side (`ep-swing-intelligence`, `vaa-approach-angle-study`) — but here it plays out inside a single
athlete's recovery from the same injury, twice. The raw physical tool
(sprint speed) degrades in a step-wise, trackable way that closely follows
...
- **Injury dates:** July 10, 2021 (right ACL tear) and May 26, 2024
  (left ACL tear, surgery June 6, 2024) — some retrospective sports
  articles report July 20, 2021 for the first injury; this appears to be
  a copied error, as contemporaneous same-day reporting from MLB.com and
  Sports Illustrated, both dated July 10, 2021, confirms the correct date.
- **Sprint speed:** Baseball Savant Sprint Speed leaderboard, pulled by
  season, 2018-2026. Cross-checked directly against Baseball Savant's
  player page Statcast Running Statistics table.
...
- **2026 is a partial season.** The sharp SB success rate drop this year
  could partially reflect a smaller sample of attempts (23) compared to
  prior full seasons, and the sprint speed and SB figures both reflect
  the point in the season this analysis was pulled — refresh against
  Baseball Savant / Baseball-Reference before citing as a final-season
  number.
...
Specialist. [Emerson Performance](https://github.com/ejimenezperformance) · [@emersonperformance](https://instagram.com/emersonperformance)
